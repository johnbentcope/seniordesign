...
void setup() {
  ...
  for (int i = 0; i < 3; i++)
  {
    s[i] = createGraphics(width,height, P3D);
  }
  r = new Rift();
}
...
void draw() {
  ...
  for(int i = 0; i < 3; i ++)
  {
    ...
    for (int j = 0; j < numPoints; j++)
    {
      ...
      s[i].pushMatrix();
      s[i].translate(vscale*sin(scale*theta)*cos(scale*rho),
      ...
      theta+= HALF_PI;
      s[i].pushMatrix();
      s[i].translate(1.5*vscale*sin(scale*theta)*cos(scale*rho),
  ...
  image(s[cameraToggle],0,0);
  
}

void mousePressed() {
  cameraToggle = (cameraToggle + 1) % 3;
}
